...
;                                             99 - by prescribed values (see 'subtr_levels' parameter)
;                                             see also using of 'subtr_data' parameter
;   subtr_levels  (in/out) (1-D real)       - if mode = 99, should be the array of the length = number of frequencis (otherwise err_code = 1 or 2 will be set). 
...
;   subtr_data    (in)     (3-D real)       - if specified, shouild be of the same size as maps in 'maplist' (1,2-D - coordinates, 3-D - frequencies). If sizes of 'subtr_data'
;                                             are differs from the calculated data ones, err_code will be set to 11.  
;                                             Will be subtract from the maps in 'maplist' before scan calculation. 
;                                             'subtr_mode' and 'subtr_levels' will be ignored if 'subtr_data' is set.
;
;   freq_set      (out)    (1-D double)     - set of frequencies used for scan calculation. See Comment (*)
;   out_data      (out)    (3-D double)     - radiomaps to calculate scans ((1,2-D - coordinates, 3-D - frequencies)). See Comment (*) 
;   out_index     (out)    (1-D struct)     - corresponding index stuctures (for each frequency). See Comment (*)
;   err_code      (out)    (integer)        -  0:    if everything is correct
;                                           -  1:    subtr_mode = 99 and 'subtr_level' is missed
;                                           -  2:    subtr_mode = 99 n_elements(subtr_level) is not equal to the number of frequiencies
;                                           - 11:   'subtr_data' is set and it's sizes are not equal to the radiomap ones
;
; Comments:
;   (*)  If 'freq_set' doesn't set, frequencies will be from the 'maplist'. 'freq_set' in this case will be the same set as in 'maplist'.
;        If 'freq_set' is set, 'out_data' and 'out_index' are filtered according selected frequencies 'freq_set'
...
if n_elements(subtr_data) ne 0 then begin
    szd = size(data)
    szs = size(subtr_data)
    if (szd[1] ne szs[1] || szd[2] ne szs[2] || szd[3] ne szs[3]) then return
endif

default, subtr_mode, 1
if subtr_mode ne 99 then data = asu_gxm_subtract_qs(data, index, mode = subtr_mode, levels = subtr_levels, err_code = err_code) ; , verbose = 1)
